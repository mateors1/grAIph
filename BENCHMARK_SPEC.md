...
**History:** Derived from 50+ benchmark runs over three months. Methodology
refined iteratively as new failure modes were discovered — including failure
modes of the benchmark itself (see v1.1 and the Step 0 precondition).
...
| **1.1** | **2026-07-17** | **Step 0 leak-detection precondition added (mandatory). All previously reported byte-identical counts retracted: forensic audit (2026-07-16) proved byte-identical output files in every auditable run (V33–V50) were pipeline fallback copies of the imported original, not model generations; earlier runs cannot be retro-audited and are retracted as unverifiable. Metrics 1 and 2 now require provenance verification. FALLBACK-COPY added to the bug catalog.** |

---

## Part 0 — Step 0: Leak-Detection Precondition (mandatory as of v1.1)

**Why this exists.** A forensic audit completed 2026-07-16 found that every
byte-identical output file in every auditable benchmark run (V33–V50) was written
by pipeline infrastructure, not generated by the model: when codebase import seeds
a node's state with the original file content, and that node's generation later
produces no output, the batch-repair rollback path flushed the seeded original
bytes to the output directory. Byte-identical files correlated 100% with
`filesGenerated: 0` generation events; in the deepest-audited run, output file
timestamps match the batch-repair event to the second. The byte-identical metric
was therefore measuring a leak path, not generation fidelity. This precondition
is the mechanical guard that makes the metric trustworthy again. **A byte-identical
file is a red flag, not a win, until it passes this gate.**

**The gate.** Before any scoring proceeds, every run MUST pass all three checks:

1. **Output provenance check.** Compute the hash of every ground-truth file and
   compare against every output file. For any byte-identical pair, the owning
   node's generation log must show positive generation evidence (final
   `node:done` event with `filesGenerated > 0`, and the node's recorded content
   hash must not be the import-seed placeholder). A byte-identical file without
   generation evidence is classified `FALLBACK-COPY`: it is excluded from
   Metrics 1 and 2, scored as a missing file for generation-quality purposes,
   and logged separately as pipeline contamination.
2. **Prompt provenance check.** Every assembled context packet sent to any agent
   (Coder, QA, Conformance) is scanned for ground-truth file content. Any match
   outside explicitly permitted channels — Track D (poisoned, by definition), or
   Track B sealed metadata (which is derived, never verbatim) — fails the run
   before scoring. A failed run is INVALID and is not entered in any leaderboard.
3. **Generation-evidence logging present.** Runs must be executed with
   benchmark event logging enabled. A run whose generation events were not
   captured cannot pass checks 1–2 and is not a valid benchmark run. This is
   why pre-V33 byte-identical counts are retracted as unverifiable rather than
   re-scored: the logs needed to audit them were not retained.

This is the same class of fix as earlier mechanical validity guards in this
project's history (see v0.2): a non-optional precondition, enforced before any
quality judgment, so that a compromised metric fails loudly instead of
inflating silently.
...
Results not entered in clean leaderboard. The previously reported "pipeline
determinism ceiling" (100% byte-perfect on external machine, May 2026) predates the
Step 0 provenance gate and is **retracted as unverified (2026-07-17)** — a 100%
byte-identical result is exactly the signature the fallback-copy leak produces, and
that run cannot be retro-audited.
...
**Precondition (v1.1):** Only files passing the Step 0 output provenance check
count toward this metric. Files classified `FALLBACK-COPY` are excluded and scored
as missing.
**Reports as:** `X/37 (Y%)` — provenance-verified count only.
...
Use Metric 2 to distinguish. Note that a genuine top-fidelity generation is
typically *near*-identical (e.g. line-ending or trailing-newline differences from
the output envelope) — exact byte identity at scale should always trigger Step 0
scrutiny.
...
**Precondition (v1.1):** Same as Metric 1 — Step 0 provenance-verified files only.
**Reports as:** `X/37 (Y%)`
...
| `FALLBACK-COPY` | Byte-identical output with no generation evidence (`filesGenerated: 0`) — batch-repair flushed the import-seeded original to output | Inflates byte-identical metric with leaked ground truth; invalidates the metric for the run if uncaught | 🔴 Benchmark-invalidating |
...
> ⚠️ **Under re-evaluation (2026-07-17):** both endpoints of this comparison are
> v1 judge-estimated scores from runs that predate the Step 0 provenance gate and
> cannot be retro-audited. The byte-identical-weighted dimensions (~30% of the v1
> composite) are unverifiable for these runs. The direction of the delta is
> supported by non-byte-identical evidence (compile errors, truncation, decorator
> drops improved dramatically between V10 and V23), but the +4.9 magnitude must be
> re-established under v1.1 methodology before being cited.

The V10→V23 Flash Lite comparison (+4.9 points, same model) was argued as a valid
pipeline improvement claim DESPITE different pipeline versions because:
...
Future runs should reproduce this before/after control under v1.1 (Step 0 enforced)
on additional models to re-establish and triangulate the finding.
